...
const char* wifi_ssid = "rb";
const char* wifi_passwd = "987654321*";
...
void init_led_resource()
{
    led_resource.id = 0;
    led_resource.gpio = 0;
    led_resource.status1 = LOW;
}

int init_wifi() {
    int retries = 0;

    Serial.println("Cnectando ao WiFi......");
...
void get_leds() {
    ...
    if (led_resource.id == 0)
        ...
        jsonObj["status1"] = led_resource.status1;
...
void post_put_leds() {
    ...
    if (!jsonBody.success()) {
        Serial.println("erro na passagem do json");
        ...
        if (http_rest_server.method() == HTTP_POST) {
            ...
            if (jsonBody["id"] == led_resource.id) {
                ...
                digitalWrite(led_resource.gpio, led_resource.status1);
            }
            else
              http_rest_server.send(404);
        }
...
void setup(void) {
    ...
    if (init_wifi() == WL_CONNECTED) {
        ...
        Serial.print("Erro ao conectar a: ");
    ...
    Serial.println("HTTP REST Server");
}
